...
bool CConnman::Start(CScheduler& scheduler, const Options& connOptions)
{
    ...
    if (m_use_addrman_outgoing) {
        ...
        m_anchors = ReadAnchors(gArgs.GetDataDirPath() / ANCHORS_DATABASE_FILENAME);
...
void CConnman::StopNodes()
{
    if (fAddressesInitialized) {
        ...
        if (m_use_addrman_outgoing) {
            ...
            DumpAnchors(gArgs.GetDataDirPath() / ANCHORS_DATABASE_FILENAME, anchors_to_dump);
        }
    }
...
void CaptureMessage(const CAddress& addr, const std::string& msg_type, const Span<const unsigned char>& data, bool is_incoming)
{
    ...
    fs::path base_path = gArgs.GetDataDirPath() / "message_capture" / clean_addr;
