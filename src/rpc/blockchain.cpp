...
static RPCHelpMan dumptxoutset()
{
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    const fs::path path = fsbridge::AbsPathJoin(gArgs.GetDataDirPath(), request.params[0].get_str());
    // Write to a temporary path and then move into `path` on completion
    // to avoid confusion due to an interruption.
    const fs::path temppath = fsbridge::AbsPathJoin(gArgs.GetDataDirPath(), request.params[0].get_str() + ".incomplete");
