...
BOOST_AUTO_TEST_CASE(dbwrapper)
{
    ...
    for (const bool obfuscate : {false, true}) {
        fs::path ph = m_args_manager.GetDataDirPath() / (obfuscate ? "dbwrapper_obfuscate_true" : "dbwrapper_obfuscate_false");
...
BOOST_AUTO_TEST_CASE(dbwrapper_basic_data)
{
    ...
    for (bool obfuscate : {false, true}) {
        fs::path ph = m_args_manager.GetDataDirPath() / (obfuscate ? "dbwrapper_1_obfuscate_true" : "dbwrapper_1_obfuscate_false");
...
BOOST_AUTO_TEST_CASE(dbwrapper_batch)
{
    // Perform tests both obfuscated and non-obfuscated.
    for (const bool obfuscate : {false, true}) {
        fs::path ph = m_args_manager.GetDataDirPath() / (obfuscate ? "dbwrapper_batch_obfuscate_true" : "dbwrapper_batch_obfuscate_false");
        CDBWrapper dbw(ph, (1 << 20), true, false, obfuscate);
...
BOOST_AUTO_TEST_CASE(dbwrapper_iterator)
{
    // Perform tests both obfuscated and non-obfuscated.
    for (const bool obfuscate : {false, true}) {
        fs::path ph = m_args_manager.GetDataDirPath() / (obfuscate ? "dbwrapper_iterator_obfuscate_true" : "dbwrapper_iterator_obfuscate_false");
        CDBWrapper dbw(ph, (1 << 20), true, false, obfuscate);
...
BOOST_AUTO_TEST_CASE(existing_data_no_obfuscate)
{
    ...
    fs::path ph = m_args_manager.GetDataDirPath() / "existing_data_no_obfuscate";
...
{
    // We're going to share this fs::path between two wrappers
    fs::path ph = m_args_manager.GetDataDirPath() / "existing_data_reindex";
    create_directories(ph);
...
{
    fs::path ph = m_args_manager.GetDataDirPath() / "iterator_ordering";
...
BOOST_AUTO_TEST_CASE(iterator_string_ordering)
{
    ...
    fs::path ph = m_args_manager.GetDataDirPath() / "iterator_string_ordering";
    CDBWrapper dbw(ph, (1 << 20), true, false, false);
...
BOOST_AUTO_TEST_CASE(unicodepath)
{
    ...
    fs::path ph = m_args_manager.GetDataDirPath() / "test_runner_₿_🏃_20191128_104644";
